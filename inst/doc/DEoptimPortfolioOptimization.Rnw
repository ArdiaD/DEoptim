...
Because the resulting optimization model is a non-convex programming problem with multiple local optima, 
\pkg{DEoptim} is more apt in solving this problem than gradient-based 
...
%\VignetteDepends{quantmod, PerformanceAnalytics}
...
from \pkg{PortfolioAnalytics} \citep{PortAnalytics}.\footnote{All packages are readily available from CRAN, except for \pkg{PortfolioAnalytics} 
...
Second, we use the function \code{random_portfolios} (taken 
from the \pkg{PortfolioAnalytics} \citep{PortAnalytics} package) 
to generate random portfolios that satisfy all constraints. These random portfolios will be used as the initial
