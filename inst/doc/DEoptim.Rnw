...
%\VignetteDepends{vcd,lattice, grid}
...
\label{de} 
\end{equation}
where $F$ is the differential weighting factor.  Effective values of $\text{F}$ are 
...
\item \code{CR}: Crossover probability from interval [0,1]. The default
  value is \code{0.5}.
\item \code{F}: differential weighting factor from interval [0,2]. The default value is \code{0.8}.
